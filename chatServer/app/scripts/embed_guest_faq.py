...
async def main():
    ...
    dataset_path = os.path.join(os.path.dirname(__file__), "guest_faq_dataset.json")
    ...
    try:
        ...
        await col.delete_many({"type": "guest_faq"})
        logger.info("Cleared old FAQ embeddings from faq_embeddings collection.")
    ...
    logger.info(f"Generating embeddings for {len(variations)} FAQ variations...")
    ...
    for i in range(0, len(variations), batch_size):
        ...
        await EmbeddingStorageService.insert_faq_batch(inserts)
    ...
    logger.info("✅ Done! FAQ Semantic Cache initialized.")
